...
def checkAtoc(session):
    stocks = getStocks(os.getenv('high_value_stocks'))
    df = getATOC(stocks)
    df.to_csv("data/{}/{}.csv".format(session, today))
    buyDf = df[df.BP1 == session]
    buyDf.sort_values('BV1', ascending=False, inplace=True)
    print("Top BUYING")
    print(buyDf.head(20))
    buyDf = buyDf[buyDf.BV1 >= 10000]
    sellDf = df[df.SP1 == session]
    sellDf.sort_values('SV1', ascending=False, inplace=True)
    print("Top SELLING")
    print(sellDf.head(20))
    sellDf = sellDf[sellDf.SV1 >= 10000]
    message = getMessage(session, sellDf.head(20), buyDf.head(20))
    if message != "":
        sendEmail("Check {}".format(session), message, "html")
...
if __name__ == '__main__':
    ...
    if sys.argv[1] == "value":
        checkValue()
    if (sys.argv[1] == "ato") and isATO():
        checkAtoc("ATC")
    if (sys.argv[1] == "atc") and isATC():
        checkAtoc("ATC")
